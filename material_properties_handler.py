...
def print_Engineering_Constants(E_x_web, E_x_flange, G_xy_flange, G_xy_web, dim):
    A1 = dim.DIM1 # Area Flange
    A2 = (dim.DIM2 - dim.DIM4) # Area Web
    print(f"E: {(A1 * E_x_flange + A2 * E_x_web) / (A1 + A2)}")
    print(f"G: {(A1 * G_xy_flange + A2 * G_xy_web) / (A1 + A2)}")
